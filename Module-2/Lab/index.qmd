...
First, ensure you have the necessary packages installed and loaded. We will use the `dplyr`, `lubridate`, `readr`, `ggplot2`, and `tidyr` packages for our examples. Since we are using webR, this document will automatically download the packages. If you are having an issue, double check your internet.
```{r}
library(ggplot2)
library(dplyr)
library(lubridate)
