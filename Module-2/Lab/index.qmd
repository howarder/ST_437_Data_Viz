...
- **`life_expectancy`**: Synthetic life expectancy values that generally increase over time, with slower growth after 2018.
- **`birth_rate`**: Birth rate for each country and year, randomly generated within a range of 0.4 to 3.2.
- **`temperature`**: Realistic temperature in Celsius, randomly generated within a plausible range for each country and year.
...
- **`avg_life_expectancy`**: Synthetic average life expectancy values for each country, randomly generated within a realistic range.
...
#### Visualization
```{webr-r}
# Step 1: Filter the dataset for the years 2015 and 2020
slopeData <- filteredData %>%
  filter(year %in% c(2015, 2020)) %>%
  select(country, year, gdp_per_capita, avg_life_expectancy)

# Step 2: Create a slope plot
ggplot(slopeData, aes(x = year, y = avg_life_expectancy, group = country, color = country)) +
  geom_line(size = 1.5) +
  geom_point(size = 4) +
  scale_x_continuous(breaks = c(2015, 2020)) +
  labs(title = "Change in Life Expectancy vs. GDP per Capita (2015-2020)",
       x = "Year", y = "Average Life Expectancy") +
  theme_minimal() +
  theme(legend.position = "bottom") +
  geom_text(data = slopeData %>% filter(year == 2015), aes(label = country), 
            hjust = 1.1, size = 3) +
  geom_text(data = slopeData %>% filter(year == 2020), aes(label = country), 
            hjust = -0.1, size = 3)
