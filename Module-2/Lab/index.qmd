...
First, ensure you have the necessary packages installed and loaded. We will use the `dplyr`, `lubridate`, `readr`, `ggplot2`, and `tidyr` packages for our examples. If you haven't previousy used any of these packages, you'll need to install them using the `install.packages()` function.
...
url1 <- "https://howarder.github.io/ST_437_Data_Viz/Assets/Datasets/censuspop08.csv"
url2 <- "https://howarder.github.io/ST_437_Data_Viz/Assets/Datasets/CIAWorldFactbook2013.csv"
url3 <- "https://howarder.github.io/ST_437_Data_Viz/Assets/Datasets/continents.csv"

# Download the data files from the HTTPS URL and save it as
# countries.csv & countriesExtraInfo.csv
cat("Downloading the data ...\n")
download.file(url1, "censuspop08.csv")
download.file(url2, "CIAWorldFactbook2013.csv")
download.file(url3, "continents.csv")
...
population <- read_csv("censuspop08.csv", show_col_types = FALSE)
factbook2013 <- read_csv("CIAWorldFactbook2013.csv", show_col_types = FALSE)
continents <- read_csv("contintents.csv", show_col_types = FALSE)
```
...
#### Description of the `population` Dataset

-   **`Country`**: Country name
-   **`Continent`**: Continent country is located in
-   **`CIA_Def`**: Country categorized as "developed", "transition", or "developing", as determine by the CIA
-   **`1969`**: Population in 1969
-   **`1970`**: Population in 1970
-   ...
-   **`2008`**: Population in 2008    

#### Description of the `factbook2013` Dataset

-   **`Country`**: Country name
-   **`Pop2013`**: Population in 2013
-   **`Area_sqkm`**: Country's area in square kilometers
-   **`PopGrowthRate`**:
...
You are tasked with visualizing trends in health expenditure and life expectancy in countries with populations of at least five million people.

##### Filtering Process

```{r}
filteredData <- factbook2013 |>
  filter(Pop2013 > 5000000)
...
```{r}
ggplot(filteredData, aes(x = HealthExpend, y = LifeExpectancy)) +
  geom_point(size = 2) +
  labs(title = "Health Expenditure vs. Life Expectancy in Large Countries",
       x = "Health Expenditure (percent of GDP)", y = "Life Expectancy (years)") +
  theme_minimal()
```

##### What did we do here?

By focusing on specific countries, filtering allows for more targeted and relevant visualizations, making it easier to analyze trends and patterns specific to the context.
...
1.  **Population Size in Africa**:
    -   **Filter**: Only countries where the net migration rate is negative (i.e. more people have emigrated than immigrated)
    -   **Visualize**: 
...
Summarizing, often done by aggregating data by a variable(s), can be used to produce summary statistsics.
...
When data are spread across multiple datasets, we may need to join datasets together to produce the desired summaries or visualizations.

When joining datasets, we'll consider the main or primary dataset to be the `x` dataset and the secondary dataset to be `y`.

There are a few join functions we'll use:

-   `inner_join()` joins `x` and `y` based on a key. Only observations in `x` that have a matching key `y` are kept. 
...
# Check the first 5 rows and last column
head(countriesDs)
```
